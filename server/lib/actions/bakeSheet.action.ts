...
export const generateBakeSheetFromOrders = async (
  ...
  const reservedStockForOtherDates: Record<string, number> = {};
  ...
  const otherProducts = await Product.find({}).populate("ingredients.ingredient", "name unit");
  for (const otherOrder of otherOrders) {
    for (const item of otherOrder.items) {
      const product = otherProducts.find(
