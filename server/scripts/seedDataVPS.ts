...
async function seedDataVPS() {
  try {
    ...
    const lotsData = [
      ...
      },
      {
        ...
        lots: [
          {
            quantity: 20,
            expiryDate: addDays(new Date(), 150),
    ...
    for (const productData of productsData) {
      ...
      let category = await Category.findById(productData.category);
      if (!category) {
      ...
      let product = await Product.findOne({ productId: productData.productId });
    ...
    // Product recipes
    const productRecipes: Record<string, Array<{ ingredientName: string; quantity: number }>> = {
      "Sweet Cake": [
